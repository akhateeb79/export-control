-- The unauthenticated login lookup must not expose recoverable MFA material.
DROP FUNCTION lookup_auth_credential(TEXT);

CREATE FUNCTION lookup_auth_credential(p_email TEXT)
RETURNS TABLE (
  user_id UUID,
  tenant_id UUID,
  email TEXT,
...
AS $$
  SELECT c.user_id, c.tenant_id, c.email, c.password_hash
